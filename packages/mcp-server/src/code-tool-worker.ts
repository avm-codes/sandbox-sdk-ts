...
import { SandboxSDK, ClientOptions } from '@avmcodes/sandbox-sdk';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: SandboxSDK)`
    : `const run: (${functionSource.client}: SandboxSDK) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(
  [
    ...
    'client.sandboxes.deleteAll',
    'client.sandboxes.download',
