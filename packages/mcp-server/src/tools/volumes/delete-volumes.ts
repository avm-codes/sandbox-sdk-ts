...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'delete',
  httpPath: '/v1/volumes/{id}/delete',
};

export const tool: Tool = {
  name: 'delete_volumes',
  description:
    "When using this tool, always use the `jq_filter` parameter to reduce the response size and improve performance.\n\nOnly omit if you're sure you don't need the data.\n\n\n\n# Response Schema\n```json\n{\n  $ref: '#/$defs/volume_delete_response',\n  $defs: {\n    volume_delete_response: {\n      type: 'object',\n      properties: {\n        id: {\n          type: 'string'\n        },\n        message: {\n          type: 'string'\n        },\n        type: {\n          type: 'string',\n          enum: [            'volume',\n            'snapshot'\n          ]\n        }\n      },\n      required: [        'id',\n        'message'\n      ]\n    }\n  }\n}\n```",
  inputSchema: {
    type: 'object',
    properties: {
      id: {
        type: 'string',
        description: 'Volume ID or Snapshot ID',
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
...
export const handler = async (client: SandboxSDK, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.volumes.delete(id)));
  } catch (error) {
    if (isJqError(error)) {
